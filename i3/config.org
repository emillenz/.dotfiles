...
- browser :: it hides away unsed and distracting sidebars of overbloated websites.
- gapsize :: since we can't assume what windows will be displayed, we use a compromise that will work for the majority of programs & websites.  if it is insufficient, the user can just toggle the gaps.
#+begin_src conf
# 1920x1080 display
set $GAPSIZE 400
...
* marks
- use mnemonic UPPERCASEmarks for our most used programs (if accessing a temporary window often in the current sessino, mark it with ~$MOD+m~ and goto to it using ~$MOD+apostrophe~).
