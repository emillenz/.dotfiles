...
- don't use evil-ex, instead make it less complex and use emacs ~M-x~. use the vim-bindings only for text navigation/editing.
  - :%s/ :: use enhanced ~query-replace-regex~ which is more interactivy powerful/flexible than vim's replacement.
  ...
  - ! :: use emacs: async shell cmd: ~M-&~, and ~shell-command-on-region~ (already remapped)
- s/S :: vim's s/S is useless, since it is a duplicate of (x/C).  we introduce instead a powerful operator: surround.  anytime you need to add/remove/change surrounding delimiters, this will make you more efficient & precise.
#+begin_src elisp
(map! :after evil
