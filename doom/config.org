...
- s/S :: vim's ~s/S~ is useless, since it is a duplicate of the bindings: ~x~ and ~C~.  query-replace needs to be easily accessible (used all the time for renaming var's etc.) so it gets a top level binding and has directional forward/backward mapping like vim's: ~/?~.
- gs :: we introduce a powerful operator: ~evil-surround~.
