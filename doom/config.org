...
** global marks
- mark the files you find yourself alternating between with global-marks (if across projects), and jump to the exact file you want directly via the mark.
...
- save evil marks when saving project session to disk: ~doom/save-session~.  save state for each project when done and then restore when reopening.
#+begin_src elisp
...
- we use [[kbd:][SPC m]] and [[kbd:][M]] since harpoon marks are analogous to vim marks.
#+begin_src elisp
(after! harpoon
...
* lispyville: editing lisp in vim
