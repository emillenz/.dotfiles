...
- s/S :: vim's ~s/S~ is useless, since it is a duplicate of the bindings: ~x~ and ~C~.  we introduce instead a powerful operator: surround.   use it to add/remove/change surrounding delimiters.
#+begin_src elisp
(map! :after evil
...
** lisp
- disable confusing lispy insert bindings: ~[]~ (goes against vim philosophy, we navigate using normal-mode, we type using insert-mode)
#+begin_src elisp
