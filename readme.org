...
* keybindings
** general notes
- [[kbd:][ctrl]] :: mapped to [[kbd:][capslock]].  additionally we use [[kbd:][C-[]] instead of [[kbd:][esc]].  this greatly increases ergonomics and speed for all ctrl-bindings.
  - when bill-joy orignially developed VI, the terminal's keyboard, had [[kbd:][esc]] on todays [[kbd:][tab]] and [[kbd:][ctrl]] on [[kbd:][caps.]]  this is where [[kbd:][ctrl]] should have always stayed at.  it is more ergonomic and faster than using the incredibly unergonomic and hard to reach [[kbd:][backspace.]]  hence we implement these bindings in all apps.
- deleting characters :: instead of spamming unergonomic [[kbd:][backspace]], get in the habit of always directly using [[kbd:][C-h/w/u]] instead. - typing technique :: get into the habit of proper technique in touch typing.  staying on the homerow, typing with little pressure as needed and not using excess force.  this includes touchtyping all numberrow symbols, brackets & braces.  to enforce this, spraypaint the keycaps blank in order to force you to never look at the keyboard again.  force yourself to use the right finger for the right key (right side symbols :: all with the pinky)
