...
- we use [[kbd:][C-l]] over [[kbd:][zz]] (same as shell + [[kbd:][ctrl]] bindings are already for screen-movements: [[kbd:][C-u/d/e/y]])
- HACK :: in c-modes it gets overridden by an obscure lambda ~ccls-navigate~.  must set with hook to override.
#+begin_src elisp
