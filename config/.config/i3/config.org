...
set $SCROT scrot --file "$HOME/Pictures/screenshots/%Y-%m-%d_T%H-%M.png" --exec 'dunstify "screenshot" "$f"'
#+end_src
...
- dmenu_run :: start process in a terminal if the user-input ends on ~!~
#+begin_src conf
...
bindsym $MOD+1            $SHCMD $BIN/window-goto 1
bindsym $MOD+2            $SHCMD $BIN/window-goto 2
bindsym $MOD+3            $SHCMD $BIN/window-goto 3
bindsym $MOD+4            $SHCMD $BIN/window-goto 4
#+end_src
...
mode "leader" {
        bindsym $MOD+a $SHCMD $BIN/audio-output-menu $DMENU_THEME; $ESC
        bindsym $MOD+b $SHCMD $BIN/bluetooth-menu $DMENU_THEME; $ESC
        bindsym $MOD+k $SHCMD $BIN/killprocess-menu $DMENU_THEME; $ESC
        bindsym $MOD+d $SHCMD sh $HOME/.screenlayout/$(fd '.' --base-directory $HOME/.screenlayout | dmenu -p 'screenlayout > ' $DMENU_THEME); $ESC
