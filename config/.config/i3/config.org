...
# <pixel> hides titlebars when only window on the workspace
...
# in case you must display workspace temporarily without gaps.
...
- each workspace has it's own optimal gap size for a focused & ergonomic view (browser's fullscreen, text-based programs width constrained).
- if you are forced to work with a desktop environment, you can easily replicate this config by manually puttin clients on separate workspaces and using Alt+<number> to swtich.
...
- workspaces are powerful, since they allow multiple windows of the same class to be opened on the same workspace (eg. multiple instances of zathura).  and they allow per window gaps/fulscreen configuration.
...
- gapsize :: 1920x1080 display (adjust ifdifferent screenresolution)

#+name: mark-classes
#+begin_src elisp
(let ((gapsize 500)
      (mark-classes '(;; ("name" "key" "wmclass" "startcmd" "gaps")
                      ("term"
                       "t"
                       "Alacritty"
                       "alacritty --title term --command tmux new-session -n term -A -s main"
                       t)
  ...
  (mapconcat (lambda (tuple)
               (let* ((name (nth 0 tuple))
                      ...
                      (class (nth 2 tuple))
                      (cmd (nth 3 tuple))
                      (gaps-flag (nth 4 tuple)))

                 (mapconcat #'identity
                            (remove nil (list (format "# WORKSPACE: %s" (upcase name))

                                              (when gaps-flag
                                                (format "workspace %s gaps horizontal %s"
                                                        name
                                                        gapsize))
