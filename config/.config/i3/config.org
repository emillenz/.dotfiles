...
- i3 as rofi fallback :: incase rofi not installed yet or broken-config.
  - tipp :: in ~rofi~ you can use [[kbd:][C-<return>]] to execute your input as a shell command.
#+begin_src conf
...
bindsym $MOD+Return       $SHCMD rofi -show drun || i3-dmenu-desktop
bindsym $MOD+shift+Return $SHCMD rofi -show run
...
mode "leader" {
        bindsym $MOD+a $SHCMD audio-output-menu; $ESC
        bindsym $MOD+b $SHCMD bluetooth-menu; $ESC
        bindsym $MOD+k $SHCMD killprocess-menu; $ESC
