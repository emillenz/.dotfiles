...
** line numbers
- we make extensive use of relative line motions (eg: ~12dd~ etc.) , hence we need ~visual~ line-numbers. (~visual~ also works with folded code (eg. ~org-mode~)).
#+begin_src elisp
...
- note :: you must install the formatters in ~apheleia-mode-alist~ for the languages you use.
#+begin_src elisp
