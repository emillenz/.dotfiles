...
- REPL-buffer :: for repl-coupled languages (python, zsh, clojure) we already have efficient bindings to jump back and forth between the ~REPL~ / ~shell~ buffer and our file(s).  (on ~localleader~ map for the mode).
