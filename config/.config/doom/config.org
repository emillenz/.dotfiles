...
- minibuffers :: at bottom, consistent with ~minibuffer-prompt~, ~whichkey~, etc.  use ~doom/window-enlargen~ if you need to view it large.
...
some modes don't obey ~display-buffer-alist~, so we set the same-window-behaviour in their special way.
#+begin_src elisp
(after! org
...
#+end_src
** line-numbers
- we make extensive use of motions (eg: ~d2}~, ~d12j~, etc), hence we need ~visual~ line-numbers.  (+ we don't hard-fill our text in prose, instead we use ~visual-fill-column-mode~, which means we need ~'visual~ instead of ~'relative~ numbers)
#+begin_src elisp
...
- [[kbd:][C-j]] :: inverse of: [[kbd:][J]], ~electric-newline-and-indent~.  this is really convenient to eg. split function arguments onto newlines.
- note :: don't use the common remap: ~evil-next-line~ to ~evil-next-visual-line~ (or ~prev~).  since this causes inconsisten number prefix line-motions (eg: ~12dd~ deletes by full lines, but ~12dj~ deletes by visual lines)
...
- note :: when you use motions, don't use motions like ~5dd~ (or ~yy~, ~cc~). instead always opt to using ~5d<motion>~ (eg: ~5dj~).  this technique is more powerful anyways, since it allows you to go bidirectional.  (additionally using ~dd~ etc. causes confusing behaviour when referencing the line numbers (offset by one) and with our remaps might not even correspond to the right line number anyway.)
#+begin_src elisp
...
- ~evil-visual-block~ :: still enabled, since this is not replaceable by motions.
...
** org/keybindings
- we don't hard format (using ~evil-fill~) long lines in prose (~org-mode~).  since it makes sense for the content (eg. a long org list item) to be coherent as a single block of text.  we use ~visual-fill-column~ to display lines right.  navigate within a long line using ~evil-ex-search~ and ~evil-org-forward-sentence~.
#+begin_src elisp
