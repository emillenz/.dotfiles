...
- localleader :: [[kbd:][SPC m]] for ~major-mode~.  it is nice an egonomic (at least on ~dvorak & querty~).
...
** buffer goto (harpoon concept, but for frequently used buffers)
- shell :: for ~shell~ session buffers (ruby, zsh,...) we have bindings that bring forth the buffer.  eg [[kbd:][SPC o t]] to goto the terminal.
- compilation :: in any projects that we compile, we frequently switch to the ~*compilation*~ buffer.
#+begin_src elisp
