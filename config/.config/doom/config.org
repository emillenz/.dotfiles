...
** line numbers
- we make extensive use of relative line motions (eg: ~12dj~ etc.) , hence we need ~visual~ line-numbers. (~visual~ also works with folded code (eg. ~org-mode~)).
#+begin_src elisp
...
- consistency :: the only reliable, repeatable, transportable way to ensure that indentation remains consistent across viewing environments is to indent code using only spaces.
...
- note :: install the formatters in ~apheleia-mode-alist~ for the languages used.
#+begin_src elisp
...
- history :: [[kbd:][j, k]] is what bash's vi-mode uses by defaults, it is a good and ergonomic default.  ~evil~ expects us to use unergonomic/inaccessible [[kbd:][up, down]] by default.  (for ~comint-mode~ must use [[kbd:][ctrl]] modifier, since it is a regular buffer).
...
we use [[kbd:][C-l]] over [[kbd:][zz]] (same as shell + ctrl-bindings are already for screen-movements: [[kbd:][C-u/d/e/y]]), but ~c~ modes don't respect that for some reason.
#+begin_src elisp
...
- after spending considerable amounts of time using emacs with ~viper-mode~ (~vi~ has no visual selections), i came to the conclusion that using ~visual-mode~ in ~vim~ actually encourages less efficient behaviour (manually selecting text even though a motion would have been more efficient).
...
- this enforces us to use vim more efficiently.  additionally it comes with the benefit of making our workflow more compatible if we are working on a different setup without doom, and only vanilla emacs+viper-mode (or just ~vi~ on a server) is available.
- if we absolutely must use a visual selection, we can still use [[kbd:][C-space]] (emacs binding).
- to paste over some text in visual mode, delete the region, then paste and use [[kbd:][C-p]] ~evil-paste-pop~ to insert what was originally yanked.
